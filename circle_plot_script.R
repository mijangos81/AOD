...
circos <- function (mapping=mapping, xc=400, yc=400, R=400, W=W,  
  ...
  if (type == "heatmap"){
    ...
    rbPal <- colorRampPalette(c("darkslategray4","ivory","deeppink3"))
    ...
    if (col.bar){
      if (col.bar.po == "topleft"){
        color.bar(350,350,360,450, v.min=dat.min, v.max=dat.max);
      }
  ...
  if (type == "ss"){
    ...
    for (chr.i in 1:chr.num){
      ...
      # background line
      if (B){
        draw.arc.pg(xc, yc, v1, v2, 190, 350, col="gray90");
      } else {
          draw.arc.s(xc, yc, r, v1, v2, col=colors()[245], lwd=lwd);
      }
    }
    ...
    for (chr.i in 1:chr.num){
      ...
      for (i in 1:nrow(dat)){           
        ...
        myColours1 = "deeppink3"
        myColours2 = "darkslategray4"
        
        myColoursAlpha1 <-  colorRampPalette(c("darkslategray4","ivory","deeppink3"),alpha=F)(dat.max/2)
          # colorRampPalette(c("darkslategray4","ivory","deeppink3"),alpha=T)(dat.max/2)
          #add.alpha(myColours1, alpha=0.5)
          #colorRampPalette(c("deeppink3","ivory"),alpha=T)(10)
          #
        myColoursAlpha2 <- "orange"
          #colorRampPalette(c("darkslategray4","ivory"),alpha=T)(dat.max/2)
          #add.alpha(myColours2, alpha=0.5)
          #colorRampPalette(c("ivory","darkslategray4"),alpha=F)(10)
        
        
         # rbPal_1 <- adjustcolor("deeppink3",alpha.f = 0.5)
         # rbPal_2 <- adjustcolor("darkslategray4",alpha.f = 0.5)

        if (d.m > 0 ){
        #if (d.m > dat.me){
          col <- myColoursAlpha1[my.v]
            #myColoursAlpha2[my.v]
            # rainbow(10, alpha=0.5)[1];
        } else {
          col <- myColoursAlpha1[my.v]
            # rainbow(10, alpha=0.5)[7];
        }
        ...
        the.v  <- scale.v(d.m, my.R1, my.R2, dat.min, dat.max);
        the.c  <- scale.v(d.m, 0.2, the.cex, dat.min, dat.max);
...
color.bar <- function(xl, yb, xr, yt, v.min, v.max) {
  ...
  lut   <- colorRampPalette(c("darkslategray4","ivory","deeppink3"))(100);
