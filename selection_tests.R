...
for (i in 1:length(folders)){
  ...
  # from the R package coda. we Used the tutorial on how to use BayeScan & BayeScenv accessed from: 
  ...
  files_B_N_emp <- dir(path.folder_sim,pattern = "^fdist")
  ...
  if(B_N_test==TRUE){
    ...
    B_N_files_neutral <- B_N_files_neutral[sample(x=1:nrow(B_N_files_neutral) ,size = 5000),]
    ...
    for(i in 1:length(B_N_files_emp_2)){
      ...
      colnames(FST.empDF_res) <- c("alpha","beta")
      ...
      Shannon_fdist_emp <- B_N_files_emp_2[[i]][,c("sha_pop1","sha_pop2","shua")]
      Shannon_mean_emp <- cbind(Shannon_fdist_emp$sha_pop1,Shannon_fdist_emp$sha_pop2)
      Shannon_mean_emp <- apply(Shannon_mean_emp,1,mean)
      Shua.empDF_res <- as.data.frame(cbind(Shannon_mean_emp,Shannon_fdist_emp$shua))
      Shua.empDF_res[which(Shua.empDF_res[,2]==0),] <- NA
      Shua.empDF_res <- Shua.empDF_res[complete.cases(Shua.empDF_res),]
      colnames(Shua.empDF_res) <- c("alpha","beta")
      Shua_fdist_res <- get.pval.dataset(beta.empDF=Shua.empDF_res, beta.simDF=Shua.simDF_res,beta_max=0.7)
      Shua_fdist_res_b <- correct.pval.dataframe(dataframe=Shua_fdist_res, p.colName="p.val.cum")
      Shua_fdist_res_balancing <- Shua_fdist_res_b[Shua_fdist_res_b$tail=="L" & Shua_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,2] <- nrow(Shua_fdist_res_balancing)
      Shua_fdist_res_directional <- Shua_fdist_res_b[Shua_fdist_res_b$tail=="R" & Shua_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,10] <- nrow(Shua_fdist_res_directional)

      D_Shannon_fdist_emp <- B_N_files_emp_2[[i]][,c("one_D_alpha_pop1", "one_D_alpha_pop2","sha_diff")]
      D_Shannon_mean_emp <- cbind(D_Shannon_fdist_emp$one_D_alpha_pop1,D_Shannon_fdist_emp$one_D_alpha_pop2)
      D_Shannon_mean_emp <- apply(D_Shannon_mean_emp,1,mean)
      Sha_diff.empDF_res <- as.data.frame(cbind(D_Shannon_mean_emp,D_Shannon_fdist_emp$sha_diff))
      Sha_diff.empDF_res[which(Sha_diff.empDF_res$V2==0),] <- NA
      Sha_diff.empDF_res <- Sha_diff.empDF_res[complete.cases(Sha_diff.empDF_res),]
      colnames(Sha_diff.empDF_res) <- c("alpha","beta")
      Sha_diff_fdist_res <- get.pval.dataset(beta.empDF=Sha_diff.empDF_res, beta.simDF=Sha_diff.simDF_res,beta_max=1.01)
      Sha_diff_fdist_res_b <- correct.pval.dataframe(dataframe=Sha_diff_fdist_res, p.colName="p.val.cum")
      Sha_diff_fdist_res_balancing <- Sha_diff_fdist_res_b[Sha_diff_fdist_res_b$tail=="L" & Sha_diff_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,3] <- nrow(Sha_diff_fdist_res_balancing)
      Sha_diff_fdist_res_directional <- Sha_diff_fdist_res_b[Sha_diff_fdist_res_b$tail=="R" & Sha_diff_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,11] <- nrow(Sha_diff_fdist_res_directional)

      Shannon_pops_fdist_emp <- as.data.frame(B_N_files_emp_2[[i]][,c("sha_pops","shua")])
      Shannon_pops_fdist_emp[which(Shannon_pops_fdist_emp[,2]==0),] <- NA
      Shannon_pops_fdist_emp <- Shannon_pops_fdist_emp[complete.cases(Shannon_pops_fdist_emp),]
      colnames(Shannon_pops_fdist_emp) <- c("alpha","beta")
      Sha_pops_fdist_res <- get.pval.dataset(beta.empDF=Shannon_pops_fdist_emp, beta.simDF=Sha_pops.simDF_res,beta_max=0.7)
      Sha_pops_fdist_res_b <- correct.pval.dataframe(dataframe=Sha_pops_fdist_res, p.colName="p.val.cum")
      Sha_pops_fdist_res_balancing <- Sha_pops_fdist_res_b[Sha_pops_fdist_res_b$tail=="L" & Sha_pops_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,4] <- nrow(Sha_pops_fdist_res_balancing)
      Sha_pops_fdist_res_directional <- Sha_pops_fdist_res_b[Sha_pops_fdist_res_b$tail=="R" & Sha_pops_fdist_res_b$FDR.05==TRUE, ]
      selection_tests[i,12] <- nrow(Sha_pops_fdist_res_directional)
    }
  }
