...
#               )
Fdiff <- with(ldat,
              qlogis(Fstp_OBS) - qlogis(Fstp_EXP)
              )
...
rb <- c("blue","green","red","black")[as.numeric(as.factor(ldat$h))]
...
cframe <- data.frame(
  ...
  # cr = rainbow(5)[as.numeric(as.factor(ch))]
    cr = c("blue","green","red","black")[as.numeric(as.factor(ch))]
